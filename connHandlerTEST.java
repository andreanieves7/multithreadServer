...
public class connHandler extends Thread{
	...
	public void dataTransfer()
	{
		try{
			...
			while((lines=in.readLine())!=null) {
				if(lines.startsWith("Content-Length")) {
					String[] parse = lines.split(" ");
					size = Integer.parseInt(parse[1]);
				}
				if(lines.startsWith("Accept-Language"))
					break;
			}
			
			if(size!=0) {
				time = System.currentTimeMillis();
				FileOutputStream fos = new FileOutputStream("mana.png");
    			int read;
	        	...
	        	if(readCommand.startsWith("GET"))
	        		...
	        		putCommand(readCommand,time,totalRead);
	 ...
	 * 
	 **************************************************************************/
	private void putCommand(String readCommand, long startTime, int totalRead)
	{
		byte[] buffer = new byte[comCodes.BUFFER_SIZE];
		int read;
